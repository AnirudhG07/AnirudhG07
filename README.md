<h1 align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.herokuapp.com/?lines=Hi! This+is+Anirudh!?&center=true&size=30">
  </a>
</h1>
<p align="left"> <img src="https://komarev.com/ghpvc/?username=anirudhg07&label=Profile%20views&color=0e75b6&style=flat" alt="anirudhg07" /> </p>

- I am a student of Indian Institute of Science, Bengaluru.
- I like to do research. Interested in Lean Verification and High performance computing!
- I am a UG and trying my best to do as much as I can.
- "I am funny to chosen, randomly selected (with a nerd-biased probability distribution) individuals."
- I sometimes write some blogs about my work and interests, [check them out](https://anirudhg07.github.io/blogs)!
<br>

# Projects and Open source
Some projects I made/collabed and some Open source contributions:
- Made PastaLean and TypeInfer in Lean4. [Repo](https://github.com/siddhartha-gadgil/PastaLean)
- See [awesome-yazi](https://github.com/AnirudhG07/awesome-yazi) for yazi plugins!

# My dev preferences
```javascript
const anirudh = {
  os : ['Unix'],
  terminal_emulator: ['Ghostty', 'edex-ui'], //edex-ui Pretty!
  shell: ['zsh'], // powered by Zinit!
  code_editors: ['NeoVim', 'VS Code'], // VS Code with NeoVim extension obviously :)
  languages: ['Python', 'Rust, 'Lean4', 'C', '`Japanese`'], // LOL
  hobbies: ["Speed up programs", "Lean verify anything", "I love Terminal, CLI and TUI's. ",
  fav_tuis: ['Lazygit', 'Yazi', 'herdr'], // Just my life saviour}
```


<h3 align="left">Connect with me:</h3>
Visit my website to get to know me more: https://anirudhg07.github.io
<p align="left">
<a href="https://www.linkedin.com/in/anirudh-gupta-45a52b295/" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="anirudh gupta" height="30" width="40" /></a>
<a href="https://www.buymeacoffee.com/anirudhg07" target="_blank"><img align="center" src="https://cdn.buymeacoffee.com/buttons/default-orange.png" alt="Buy Me A Coffee" height="41" width="175"></a>  
<a href="https://kaggle.com/anirudh gupta 1729" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/kaggle.svg" alt="anirudh gupta 1729" height="30" width="40" /></a>
</p>
<br/>
